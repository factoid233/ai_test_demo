...
class DataStatistic:
    ...
    def normal_run(self):
        ...
        _class.latency_statistic()
        _class.format_latency_statistic_data()
        self.statistic_data.update(_class.latency_statistic_data)
...
class TestTypeBase:
    ...
    def latency_statistic(self):
        ...
        self.latency_statistic_data = results
        return results

    def format_latency_statistic_data(self):
        for key, value in self.latency_statistic_data.copy().items():
            self.latency_statistic_data[key] = self.format_percentage_latency(value)
        return
    ...
    @classmethod
    def format_percentage_latency(cls, data: float):
        res = '{0:>5d} ms'.format(round(data * 1000))
        return res

    def calculate_simple_field(self, _simple_df):
        """
        简单字段统计 pass 和 total的数量
        :param _simple_df:
        :return:
        """
        _statistic = {}
...
class TestType1(TestTypeBase):
    ...
    def normal_run(self):
        ...
        result_accuracy.update(self.calculate_simple_field(_simple_df))
...
class TestType4(TestTypeBase):
    ...
    def normal_run(self):
        ...
        self.data['accuracy_detail'] = accuracy_detail
        self.data['accuracy_weight_mean'] = self.accuracy_weight_mean(accuracy_detail)
        self._format_statistic_data()
    ...
    def get_need_data_from_db(self):
        ...
        self.classify_field = self.kwargs.get('classify_field')
    ...
    def accuracy_weight_mean(self, accuracy_detail: Dict[str, dict]) -> float:
        df = self.df_expect.copy()
        df[self.classify_field] = pd.to_numeric(df[self.classify_field], errors='ignore')
        df['flag'] = True
        df1 = df.groupby(self.classify_field).count()
        total = df['flag'].sum()
        res = 0
        for _type, _dict in accuracy_detail.items():
            if _type not in df1.index:
                continue
    ...
    def _format_statistic_data(self):
        data_accuracy_detail = self.data['accuracy_detail']
        accuracy_weight_mean: float = self.data['accuracy_weight_mean']
        for key, _dict1 in self.data.copy()['accuracy_detail'].items():
            data_accuracy_detail[key]['accuracy'] = self.format_percentage_accuracy(
                data_accuracy_detail[key]['accuracy'])
            data_accuracy_detail[key]['precision'] = self.format_percentage_accuracy(
                data_accuracy_detail[key]['precision'])
            data_accuracy_detail[key]['recall'] = self.format_percentage_accuracy(data_accuracy_detail[key]['recall'])
            data_accuracy_detail[key]['F1'] = self.format_percentage_accuracy(data_accuracy_detail[key]['F1'])
        self.data['accuracy_weight_mean'] = self.format_percentage_accuracy(accuracy_weight_mean)
        return


class TestType5(TestTypeBase):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def normal_run(self):
        accuracy_detail = self.accuracy_detail()
        self.data['accuracy_detail'] = accuracy_detail
        self.data['accuracy_weight_mean'] = self.accuracy_statistic(accuracy_detail)
        return self

    def accuracy_detail(self):
        classify_field = self.kwargs.get('classify_field')
        df_simple_compare = self.df_compare['simple'].copy()
        df_simple_compare[classify_field] = self.df_expect[classify_field]
        results = {}
        for _type, _df in df_simple_compare.groupby('type'):
            result = self.calculate_simple_field(_df)
            results[_type] = result.popitem()[1]
        return results
